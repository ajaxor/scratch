...
for(const cut of [false,true])for(const id of Object.keys(ROOMS)){
 if(id==='W6'&&!cut)continue;const g=new Game();if(cut){g.state.world.cabinetOpen=true;g.state.relics.push('blade');g.state.equipped='blade';g.state.world.trellisCut=true;g.state.relics.push('bell','hook');Object.assign(g.state.world.works,{archiveOpen:true,grateClear:true,rootsClear:true,tideGate:true});}travel(g,id);
 assert(g.passable(g.state.x,g.state.y),'Safe spawn '+id);assert(validSave(g.snapshot()));
 const positions=new Set();for(const o of g.data.objects){const key=o.x+','+o.y;assert(!positions.has(key),'Overlapping props in '+id+':'+key);positions.add(key);assert(WALKABLE.has(g.tile(o.x,o.y)),'Prop embedded in wall/water '+id+':'+o.id);if(!o.optional&&o.type!=='anchor'&&!(id==='T1'&&o.id==='parcel')&&(cut||!(id==='C4'&&['seed-note','root-boundary','lamp-1'].includes(o.id))))assert(path(g,(x,y)=>Math.abs(x-o.x)+Math.abs(y-o.y)<=1),'Unreachable object '+id+':'+o.id);}
...
const legacy=new Game().snapshot();delete legacy.toolRevision;delete legacy.world.tools;delete legacy.layoutRevision;delete legacy.expansion;delete legacy.world.works;legacy.room='C3';legacy.visited.push('C3');legacy.x=8;legacy.y=8;legacy.world.cabinetOpen=true;legacy.world.trellisCut=true;legacy.world.lanternAt='A1:reader-stand';legacy.relics.push('lantern','blade');legacy.equipped='blade';legacy.notes='The wheel moves something beyond the wall.';legacy.positions={'C1:cart':{x:7,y:6}};
const migrated=new Game(legacy);assert(validSave(migrated.snapshot()));const {works,tools,...originalWorld}=migrated.state.world;assert.deepEqual(originalWorld,legacy.world);assert.equal(migrated.state.notes,legacy.notes);assert.deepEqual([migrated.state.x,migrated.state.y],SPAWNS.C3);assert.deepEqual(migrated.state.positions,{});assert.equal(legacy.layoutRevision,undefined);
// Both cells of a cart must fit; pushing and undo preserve its anchor and footprint.
const cartGame=new Game();travel(cartGame,'C1');const cart=cartGame.data.objects.find(o=>o.id==='cart');assert.equal(cart.w,2);walk(cartGame,path(cartGame,(x,y)=>x===cart.x-1&&y===cart.y));assert(!cartGame.move(1,0),'Growth blocks the cart wheels');cartGame.state.world.cabinetOpen=true;cartGame.state.relics.push('blade');cartGame.equip('blade');operate(cartGame,'bramble-2');walk(cartGame,path(cartGame,(x,y)=>x===cart.x-1&&y===cart.y));const before=cartGame.snapshot();assert(cartGame.move(1,0));assert.equal(cartGame.state.positions['C1:cart'].x,cart.x+1);assert(validSave(cartGame.snapshot()));cartGame.undo();assert.deepEqual(cartGame.snapshot(),before);
