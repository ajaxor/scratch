...
import{DIRS,exitsFor}from'../dist/content.js';
export function path(g,goal){const q=[[g.state.x,g.state.y,[]]],seen=new Set([`${g.state.x},${g.state.y}`]);for(let i=0;i<q.length;i++){const[x,y,p]=q[i];if(goal(x,y))return p;for(const[dx,dy]of Object.values(DIRS)){const nx=x+dx,ny=y+dy,k=`${nx},${ny}`;if(seen.has(k)||!g.passable(nx,ny)||g.tile(nx,ny)==='D')continue;seen.add(k);q.push([nx,ny,[...p,[dx,dy]]]);}}return null;}
