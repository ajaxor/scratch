...
travel(g,'C4');const binding=g.data.objects.find(o=>o.id==='binding');assert.equal(path(g,(x,y)=>x===13&&y===6),null);operate(g,'binding');assert(g.state.world.trellisCut);walk(g,path(g,(x,y)=>x===13&&y===6));operate(g,'seed-note');assert.deepEqual(g.state.discoveries,['garden-letter']);assert(validSave(g.snapshot()));
...
const bads=[{}, {...g.snapshot(),version:1},{...g.snapshot(),x:0},{...g.snapshot(),world:{...g.state.world,lanternAt:'H:chair'}},{...g.snapshot(),positions:{'C1:cart':{x:3,y:4}}}];for(const bad of bads)assert(!validSave(bad));
