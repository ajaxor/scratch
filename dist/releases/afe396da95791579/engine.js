...
export class Game{
 constructor(saved=null){if(saved)saved=migrateSave(saved);if(saved&&!validSave(saved))throw Error('This save is not a valid opening-chapter journey.');this.state=saved?clone(saved):{version:2,expansion:1,layoutRevision:LAYOUT_REVISION,campaign:CAMPAIGN,room:'H',x:8,y:9,face:[0,-1],relics:['lantern'],equipped:'lantern',visited:['H'],connections:[],observations:[],notes:'',seconds:0,steps:0,world:{works:worksDefaults(),shadeClosed:false,cords:[false,false],cabinetOpen:false,trellisCut:false,damper:false,lanternAt:null},positions:{},discoveries:[]};this.events=[];this.history=[];}
 ...
 describe(o){const extra=describeWorks(this,o);if(extra!==null)return extra;const w=this.state.world;switch(o.type){
 ...
 interact(o=this.nearest()){if(!o||distance(o,this.state)>1){this.say('There is nothing within reach.');return;}if(interactWorks(this,o))return;const w=this.state.world;
 ...
 use(id=this.state.equipped,o=this.nearest()){if(!this.has(id))return;if(o&&distance(o,this.state)>1){this.say('It is beyond reach.');return;}if(useWorks(this,id,o))return;if(id==='lantern'){if(this.state.world.lanternAt){this.say('The lantern is resting on its stand.');return;}if(o?.type==='stand')return this.placeLantern(o);if(o)this.inspect(o);else this.say('Light runs along the joints in the stone.');return;}
...
function validate(s,legacy=false){try{
 ...
 if(!legacy){
 if(s.layoutRevision!==LAYOUT_REVISION||!validWorks(s))return false;
...
export function migrateSave(s){
 if(s&&s.expansion===undefined&&validate(s,true)){const next=clone(s);next.expansion=1;next.world.works=worksDefaults();if(next.layoutRevision===undefined){next.layoutRevision=LAYOUT_REVISION;next.positions={};[next.x,next.y]=SPAWNS[next.room];}return next;}return s;
}
