...
export function interactWorks(g,o){const w=g.state.world.works;switch(o.type){
 case'ornament':g.checkpoint();w.ornament=(w.ornament+1)%4;g.emit('click',describeWorks(g,o));return true;
 case'relief':
...
export function useWorks(g,id,o){const w=g.state.world.works;
 if(id==='blade'&&o?.type==='rootmat'){
  if(w.rootsClear){g.inspect(o);return true;}g.checkpoint();w.rootsClear=true;g.emit('cut','The dead fibres part. Water runs beneath the pale roots and away into the stone throat.');return true;
 }
 if(id==='hook'&&o?.type==='grate'){
  if(w.grateClear){g.inspect(o);return true;}g.checkpoint();w.grateClear=true;g.emit('reveal','The bill catches the hauling ring. Wet sailcloth slides onto the coping. Water can pass the rack again.');return true;
 }
